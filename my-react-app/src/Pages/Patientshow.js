...
//     axios
//       .delete(`http://127.0.0.1:8000/patients/${id}`)
//       .then(response => {
...
function PatientDetails() {
  ...
  useEffect(() => {
    ...
    // Fetch appointment data based on user ID
    axios(`https://retoolapi.dev/2jV2W1/Appointment?User_id=${id}`)
      .then((res) => setAppointmentInfo(res.data[0]))
      .catch((err) => console.log(err));
  }, [id]);

  // Function to handle the deletion of the patient account
  const handleDeleteAccount = () => {
    axios
      .delete(`http://127.0.0.1:8000/patients/${id}`)
      .then(response => {
  ...
  return (
    <>
      ...
      <div className="container-fluid">
        ...
        <div className="row" style={{ background: "#03974D" }}>
          <div className="col-lg-2 col-sm-12 my-5  d-flex flex-column align-items-center">
            <img src='https://professions.ng/wp-content/uploads/2023/07/The-Process-of-Becoming-a-Doctor-in-Nigeria-A-Roadmap2-768x768.jpg' className="border border-white border-3 rounded-2" style={{ width: '170px' }} alt="Patient" />
          </div>
          <div className="col-lg-6 lg-sm-12 my-5 text-start text-white">
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <img src="/patientlogo.png" width={"50px"} className="rounded-circle" alt="Logo" /> &nbsp;
              <h1>{patientInfo.name}</h1>
            </div>
          </div>
        </div>
            ...
            <nav className="navbar navbar-expand-lg bg-white border border-secondary" style={{ height: "100px" }}>
              ...
              <div className="collapse navbar-collapse" id="navbarNavAltMarkup">
                <div className="navbar-nav bg-white d-flex justify-content-center">
                  <button className="nav-link mx-2"><h6 style={{ color: "green" }}>Information</h6></button>
                  <button className="nav-link mx-2"><h6 style={{ color: "green" }}>Meal Plan</h6></button>
                  <button className="nav-link mx-2"><h6 style={{ color: "green" }}>Exercise Plan</h6></button>
                  <button className="nav-link mx-2"><h6 style={{ color: "green" }}>Appointment</h6></button>
                  <DropdownButton
                    id="dropdown-basic-button"
                    title="Settings"
                    variant="success"
